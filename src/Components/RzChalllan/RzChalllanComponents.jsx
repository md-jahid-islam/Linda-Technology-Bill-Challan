...
import { Link } from 'react-router-dom';
 ...
 const RzChalllanComponents = () => {
    const companyData = {
        name: "M/S RAHAT ZARIF ACCESORIES",
        logoText: "RZ",
        ...
        <div className="p-2 sm:p-4 lg:p-6 bg-gray-50 min-h-screen font-sans">           
            ...
            <div className="max-w-7xl mx-auto flex justify-between mb-4 print-hidden">
                <button onClick={handleBack}  className="bg-red-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-red-700 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 ease-in-out flex items-center space-x-2">
                    ...
                    <Link to={"/RzBill"}>M/S Rz Back To Challan </Link>
                </button>
            ...
            <div className="max-w-7xl mx-auto bg-white border-2 border-gray-400 shadow-2xl rounded-lg overflow-hidden transition-all duration-300 challan-container">
                ...
                <header className="p-4 sm:p-6 border-b border-gray-300">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        {/* Logo Text */}
                        <div className={`flex items-center justify-center w-16 h-16 border-4 border-red-600 rounded-full ${textColor} font-black text-2xl flex-shrink-0 print:border-black`}>
                            {logoText}
                        </div>
                        
                        {/* Company Details */}
                        <div className="flex-grow">
                            <h1 className="text-2xl sm:text-3xl font-extrabold text-red-600 mb-0.5 print:text-black">{name}</h1>
                            <p className="text-sm sm:text-base text-gray-800 font-bold mb-1">{slogan}</p>
                            ...
                            <div className="text-xs sm:text-sm text-gray-600 space-y-0.5">
                                ...
                                <h3 className="text-black print:text-black"><span className="">Contact:</span> {contact}</h3>
                                <h3><span className="font-semibold">E-mail:</span> {email1}, {email2}</h3>
                            </div>
                            
                            <div className="mt-2 flex justify-center sm:justify-end items-center">
                                <h2 className='font-bold text-center text-sm sm:text-base'>Website Address: {website}</h2>
                            </div>
                        </div>
                    </div>
                </header>
